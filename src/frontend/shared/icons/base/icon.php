...
abstract class Icon extends Component {
    ...
    public function render(array $props = []): string {
        $size = $this->getSize($props) / 16;
        $path = $this->path($props);

        return <<<HTML
            <svg xmlns="http://www.w3.org/2000/svg" width="{$size}em" height="{$size}em" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round">
                $path
            </svg>
        HTML;
    }
